...
pub use models::{BookDetails, BookFormat, BookSummary, ProviderCapabilities, ProviderDescriptor, SearchQuery, SearchResult};
pub use zlibrary::{ZLibraryHistoryItem, ZLibraryHistoryPage, ZLibraryProfile, ZLibraryProvider, ZLibrarySession};
...
impl ProviderRegistry {
    ...
    pub fn descriptors(&self) -> Vec<ProviderDescriptor> {
        let mut items = self.providers.iter().map(|entry| {
    ...
    pub async fn search(&self, provider_id: &str, query: SearchQuery) -> Result<SearchResult> {
        let provider = self.providers.get(provider_id).map(|entry| entry.value().clone()).ok_or_else(|| anyhow!("unknown provider: {provider_id}"))?;
    ...
    pub async fn acquisition_url(&self, provider_id: &str, id: &str, format: BookFormat) -> Result<url::Url> {
        let provider = self.providers.get(provider_id).map(|entry| entry.value().clone()).ok_or_else(|| anyhow!("unknown provider: {provider_id}"))?;
