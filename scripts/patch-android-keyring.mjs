...
// Sharing/open-with relies on the FileProvider generated by the Tauri Android
// template. Treat a template change as a build-time error instead of a runtime
// crash when the user first taps Share.
const manifestAfterPatch = await fs.readFile(manifestPath, "utf8");
const providerAuthority = "fileprovider";
if (!manifestAfterPatch.includes("androidx.core.content.FileProvider") ||
    !manifestAfterPatch.includes(providerAuthority)) {
  throw new Error("Generated Android manifest no longer declares the expected FileProvider; review the Tauri template before shipping.");
}
const resourceRoot = path.resolve("src-tauri/gen/android/app/src/main/res");
async function findXml(directory, match) {
  for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
    const target = path.join(directory, entry.name);
    if (entry.isDirectory()) {
      const found = await findXml(target, match);
      if (found) return found;
    } else if (entry.name.endsWith(".xml")) {
      const value = await fs.readFile(target, "utf8").catch(() => "");
      if (match(value)) return target;
    }
  }
  return null;
}
const providerPaths = await findXml(resourceRoot, (value) =>
  value.includes("<cache-path") || value.includes("<files-path") || value.includes("<paths")
);
if (!providerPaths) {
  throw new Error("Generated Android FileProvider paths resource is missing; sharing would fail at runtime.");
}
