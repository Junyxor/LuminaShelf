...
impl CredentialApi for Cred {
    fn set_secret(&self, secret: &[u8]) -> keyring_core::Result<()> {
        ...
        vault.with_key_and_file(|env, key, file| {
            ...
            if !edit.put_binary(env, &self.id, &ciphertext)?.commit(env)? {
                return Err(keyring_core::Error::PlatformFailure(Box::new(
                    std::io::Error::other("Android secure storage commit failed"),
                )).into());
            }
            Ok(())
        })?;
    ...
    fn delete_credential(&self) -> keyring_core::Result<()> {
        ...
        vault.with_env(|env| {
            ...
            if !editor.remove(env, &self.id)?.commit(env)? {
                return Err(keyring_core::Error::PlatformFailure(Box::new(
                    std::io::Error::other("Android secure storage deletion failed"),
                )).into());
            }
            Ok(())
        })?;
